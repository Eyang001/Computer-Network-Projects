...
class DistanceVector(Node):
    ...
    def process_BF(self):
        ...
        curr_vector = self.distance_vector.copy()
        ...
        for msg in self.messages:            
            ...
            for node_name, node_weight in node_weights.items():
                ...
                if node_weight <= -99:
                    ...
                    weight = max(int(self.get_outgoing_neighbor_weight(node_origin)) + node_weights[node_name], -99)
                    if (node_name not in self.distance_vector) or (weight <= self.distance_vector[node_name]):
                        self.distance_vector[node_name] = weight

        # Empty queue
        self.messages = []

        # TODO 2. Send neighbors updated distances
        if self.distance_vector != curr_vector:
            message = (self.name, copy.deepcopy(self.distance_vector))
            for neighbor in self.incoming_links:
                self.send_msg(message, neighbor.name)
    ...
    def log_distances(self):
        ...
        final_vector = ",".join("{}{}".format(n,v) for n,v in self.distance_vector.items())

        add_entry(self.name, final_vector)
