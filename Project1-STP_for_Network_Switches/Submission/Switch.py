...
class Switch(StpSwitch):
    ...
    def send_initial_messages(self):
        ...
        for neighbor in self.links:
            message = Message(self.root, self.dist_to_root, self.switchID, neighbor, False)
    ...
    def process_message(self, message):
        ...
        if message.root < self.root:
            ...
            self.dist_to_root = message.distance + 1
            self.switchThrough = message.origin
            self.active_link.clear()
            self.active_link.add(message.origin)

            self.process_message_helper()

        elif message.root == self.root:
            if message.distance + 1 < self.dist_to_root:
                self.dist_to_root = message.distance + 1
                self.switchThrough = message.origin
                self.active_link.clear()
                self.active_link.add(message.origin)
            ...
            elif message.distance + 1 == self.dist_to_root:
                if message.origin < self.switchThrough:
                    self.active_link.add(message.origin)
                    self.active_link.remove(self.switchThrough)
                    self.switchThrough = message.origin
                elif message.origin > self.switchThrough:
                    if message.origin in self.active_link:
                        self.active_link.remove(message.origin)
    ...
    def process_message_helper(self):
        for neighbor in self.links:
            if neighbor == self.switchThrough:
                paththrough = True
            else:
                paththrough = False
            message = Message(self.root, self.dist_to_root, self.switchID, neighbor, paththrough)
            self.send_message(message)
    ...
    def generate_logstring(self):
        ...
        log_string = ""
        for active_link in sorted(self.active_link):
            log_string += "%d - %d, " %(self.switchID, int(active_link))
